...
auto-updater restarts it and checks it stays up). To remove it for good:
    sudo systemctl disable --now <unit>      # the unit that runs wxgrid.py
The cabin display (frame.py) still reads weather/harvest-moon from Firebase,
so it is unaffected.
"""
import time
print("wxgrid.py retired - weather now runs in the cloud (Cloudflare worker + GitHub Actions). Idling.", flush=True)
while True:
    time.sleep(86400)
